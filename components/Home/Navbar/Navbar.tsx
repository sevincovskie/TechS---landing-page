    ...
    <div className={`transition-all  ${navbg ? "bg-white shadow-md" : "fixed"} duration-200  h-[12vh] z-[100] fixed w-full`}>
      <div className="flex items-center justify-between h-full w-[90%] xl:w-[80%] mx-auto">
        ...
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-xl hidden sm:block md:text-2xl text-blue-900 font-bold">
            TechS
          </h1>
        </div>
        {/*Navlinks */}
          ...
          {navLinks.map((link) => {
            return (
              <Link
                href={link.url}
                key={link.id}
                className=" text-white hover:text-blue-900 cursor-pointer font-semibold transition-all duration-200"
              >
                <p>{link.label}</p>
              </Link>
            );
